...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-projetomodelacaoapp-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-projetomodelacaoapp-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-projetomodelacaoapp-error'], {
        param: decodeURIComponent(error.response.headers['x-projetomodelacaoapp-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-projetomodelacaoapp-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-projetomodelacaoapp-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-projetomodelacaoapp-alert'], {
      param: decodeURIComponent(response.headers['x-projetomodelacaoapp-params'].replace(/\+/g, ' ')),
    });
  }
}
